 ...
 *
 * Audio wird als WAV in eine temporaere Datei geschrieben
 * und ueber expo-av abgespielt (zuverlaessiger als data URIs).
 */
...
// Gecachte WAV-Dateien (einmal generiert, dann wiederverwendet)
let cachedIncomingPath: string | null = null;
let cachedOutgoingPath: string | null = null;
...
/**
 * Generiert einen kurzen Sinus-Ton als WAV, schreibt ihn in eine
 * temporaere Datei und spielt ihn ab.
...
async function playRingtone(type: 'incoming' | 'outgoing') {
  ...
  try {
    // Pruefen ob bereits eine gecachte Datei existiert
    let filePath = type === 'incoming' ? cachedIncomingPath : cachedOutgoingPath;
    ...
    if (!filePath || !FileSystem) {
        ...
        for (let i = 0; i < numSamples; i++) {
          ...
          for (let f = 0; f < freqs.length; f++) {
            ...
            if (t >= onset) {
              ...
              const envelope = Math.exp(-localT * 3) * 0.15;
      ...
      // In temporaere Datei schreiben (zuverlaessiger als data URI)
      if (FileSystem?.cacheDirectory) {
        const tempPath = `${FileSystem.cacheDirectory}ringtone_${type}.wav`;
        await FileSystem.writeAsStringAsync(tempPath, base64, {
          encoding: FileSystem.EncodingType.Base64,
        });
        filePath = tempPath;

        // Cachen fuer zukuenftige Verwendung
        if (type === 'incoming') cachedIncomingPath = tempPath;
        else cachedOutgoingPath = tempPath;
      } else {
        // Fallback: data URI (funktioniert nicht immer auf iOS)
        filePath = `data:audio/wav;base64,${base64}`;
      }
    }
